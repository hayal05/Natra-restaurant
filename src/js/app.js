...
function buildLayouts() {
  ...
  publicScreen.className = "screen-centered";
  ...
  shell.innerHTML = `
    ...
    <div class="main">
      ...
      <div class="content">
        <div class="content-inner" id="app-outlet"></div>
      </div>
    </div>
  `;
...
async function bootstrap() {
  ...
  registerRoutes({
    "/setup": { loader: () => import("./auth/initialization.js"), public: true },
    "/login": { loader: () => import("./auth/login.js"), public: true },
  ...
  try {
    initialized = await api.auth.isInitialized();
  } catch (err) {
    pushToast(typeof err === "string" ? err : "Couldn't reach the local database.", "error", 0);
  }
...
export async function completeLogin(user) {
  ...
  try {
    ...
    pushToast(typeof err === "string" ? err : "Couldn't load settings.", "error");
  }
  navigate("/dashboard");
}
