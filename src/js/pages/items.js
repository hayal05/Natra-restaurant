...
export async function render(container) {
  ...
  // later callback (filter change, modal submit, row action) needs must be
  // captured here while `container` still holds it - never re-queried from
  // `container` afterwards.
  const els = {
    typeFilter: container.querySelector("#type-filter-header"),
    showInactive: container.querySelector("#show-inactive-header"),
    itemCount: container.querySelector("#item-count-header"),
    itemTable: container.querySelector("#item-table"),
    categoryTable: container.querySelector("#category-table"),
    addItemBtn: container.querySelector("#add-item-btn"),
    addCategoryBtn: container.querySelector("#add-category-btn"),
  };

  els.typeFilter.addEventListener("change", () => loadItems(els, categories));
  els.showInactive.addEventListener("change", () => loadItems(els, categories));

  let categories = await loadCategories(els);
  els.addItemBtn.addEventListener("click", () => openItemModal(els, categories));
  els.addCategoryBtn.addEventListener("click", () => openCategoryModal(els));
  await loadItems(els, categories);
}

async function loadCategories(els) {
  try { const categories = await withErrorToast(() => api.items.listCategories()); renderCategoryTable(els, categories); return categories; }
  catch { renderCategoryTable(els, []); return []; }
}

function renderCategoryTable(els, categories) {
  renderTable(els.categoryTable, {
    columns: [
      { key: "name", label: "Category" },
      { key: "actions", label: "Action", format: (category) => { const button = document.createElement("button"); button.className = "btn btn-secondary btn-sm"; button.textContent = "Add item"; button.addEventListener("click", () => openItemModal(els, categories, category.id)); return button; } },
    ], rows: categories, emptyMessage: "No categories yet.", getRowKey: (category) => category.id,
  });
}

async function loadItems(els, categories = []) {
  const currency = store.getState().settings?.currency ?? "USD";
  const showInactive = els.showInactive?.checked ?? false;
  const itemType = els.typeFilter?.value || null;
  ...
  if (els.itemCount) els.itemCount.textContent = countText;
  renderTable(els.itemTable, {
    columns: [
      { key: "name", label: "Item" },
...
function openCategoryModal(els) {
...
function openItemModal(els, categories, selectedCategoryId = null) {
  ...
  form.querySelector("#quick-category").addEventListener("click", () => openCategoryModal(els));
