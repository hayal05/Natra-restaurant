...
#[tauri::command]
pub fn create_waiter(db: State<Db>, full_name: String, phone: Option<String>) -> AppResult<Waiter> {
    let conn = db.lock();
    waiter_service::create_waiter(&conn, &full_name, phone.as_deref())
}
...
/// Receivables for every active waiter — feeds the dashboard's waiter cards.
#[tauri::command]
...
/// Mark all of a waiter's unsettled sales as settled. Returns the count settled.
#[tauri::command]
