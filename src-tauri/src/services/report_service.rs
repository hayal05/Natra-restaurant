...
//! sales mix, and cash flow — everything the Reports and Dashboard
//! pages need, built on top of `financial_service::calculate_profit`.
...
/// Monthly revenue/costs/expenses/profit — the core "Reports" figure.
...
/// Per-item performance (quantity sold, revenue, cost), optionally
/// scoped to a date range on the parent sale's `created_at`.
pub fn product_performance(
    ...
    let base_sql = "
        SELECT si.item_id, si.item_name, si.item_type,
               SUM(si.quantity) AS quantity_sold,
               SUM(si.line_total) AS total_sales,
               SUM(si.line_cost) AS total_cost
        FROM sale_items si
        JOIN sales s ON s.id = si.sale_id
        {WHERE}
        GROUP BY si.item_id, si.item_name, si.item_type
        ORDER BY total_sales DESC
    ";

    let rows: Vec<ProductPerformance> = match (from, to) {
        (Some(f), Some(t)) => {
            let sql = base_sql.replace("{WHERE}", "WHERE s.created_at BETWEEN ?1 AND ?2");
            let mut stmt = conn.prepare(&sql)?;
            let mapped = stmt.query_map([f, t], map_product_performance)?;
            mapped.collect::<Result<Vec<_>, _>>()?
        }
        _ => {
            let sql = base_sql.replace("{WHERE}", "");
            let mut stmt = conn.prepare(&sql)?;
            let mapped = stmt.query_map([], map_product_performance)?;
            mapped.collect::<Result<Vec<_>, _>>()?
        }
    };
    Ok(rows)
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct SalesMixEntry {
    pub item_name: String,
    pub quantity_sold: i64,
    pub total_sales: f64,
    /// Share of total revenue this item represents, 0–100.
    pub percentage_of_sales: f64,
}

/// Sales mix: each product's share of total revenue over the period.
pub fn sales_mix(conn: &Connection, from: Option<&str>, to: Option<&str>) -> AppResult<Vec<SalesMixEntry>> {
    ...
    Ok(performance
        ...
        .map(|p| SalesMixEntry {
            ...
            percentage_of_sales: if total_sales > 0.0 {
                (p.total_sales / total_sales) * 100.0
            } else {
                0.0
            },
        })
...
/// Day-by-day revenue and profit for the trailing `days` days (inclusive
/// of today). Powers the dashboard's revenue-vs-profit line chart.
///
/// Reuses `financial_service::calculate_profit` per day rather than a
/// single grouped SQL query — `days` is small (dashboard default is 14)
/// and this keeps the daily figures guaranteed consistent with every
/// other profit figure in the app instead of re-deriving the formula.
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct MonthlyCostRevenueEntry {
    pub year: i32,
    pub month: u32,
    pub revenue: f64,
    pub cost: f64,
...
/// Revenue vs. total cost (ready-made + raw-material + other expenses)
/// for each of the trailing `months` calendar months, oldest first,
/// ending with the current month. Powers the dashboard's cost-vs-revenue
/// bar chart.
...
/// Cash flow for every month of a given year: inflow (sales revenue),
/// outflow (all costs + expenses combined), and net (= profit).
...
mod tests {
    ...
    fn month_range_covers_full_calendar_month() {
        let (start, end) = month_range(2026, 2).unwrap(); // Feb 2026 (not a leap year)
    ...
    fn product_performance_and_sales_mix_add_up() {
        ...
        checkout(&mut conn, CheckoutRequest {
