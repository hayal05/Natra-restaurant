...
pub fn summary(conn: &Connection) -> AppResult<DashboardSummary> {
    ...
    let mut top_products_today = report_service::product_performance(conn, Some(&today_start), Some(&today_end))?;
    top_products_today.truncate(5);
    let mut sales_mix_this_month = report_service::sales_mix(conn, Some(&month_start), Some(&month_end))?;
    ...
    Ok(DashboardSummary { today, this_month, waiter_receivables, total_receivable, top_products_today, sales_mix_this_month, revenue_profit_trend, cost_revenue_by_month })
}
...
mod tests {
    ...
    fn summary_reflects_a_sale_made_today() {
        let mut conn = db();
        let waiter = crate::services::waiter_service::create_waiter(&conn, "Alex", None, None).unwrap();
