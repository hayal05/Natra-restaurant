...
//! ```
//!
//! Ready-Made Costs come from `sales.total_cost` (set at checkout time —
//! see sales_service). Raw-Material Costs and Other Expenses are their
//! own independent cost streams. All four figures can be scoped to an
//! optional date range for reports/dashboard.
...
pub fn create_expense(
    ...
    match expense_date {
        Some(date) => {
            conn.execute(
                "INSERT INTO expenses (category, description, amount, expense_date, note) VALUES (?1, ?2, ?3, ?4, ?5)",
                rusqlite::params![category, description, amount, date, note],
            )?;
        }
        None => {
            conn.execute(
                "INSERT INTO expenses (category, description, amount, note) VALUES (?1, ?2, ?3, ?4)",
                rusqlite::params![category, description, amount, note],
            )?;
        }
    }
...
pub struct ProfitSummary {
    pub sales: f64,
    pub ready_made_costs: f64,
    pub raw_material_costs: f64,
    pub other_expenses: f64,
    pub profit: f64,
}

/// Computes the full profit formula, optionally scoped to a date range
/// (inclusive; format 'YYYY-MM-DD' or a full datetime string — SQLite
/// string comparison handles both consistently since all our timestamp
/// columns are stored in ISO 8601 order).
pub fn calculate_profit(conn: &Connection, from: Option<&str>, to: Option<&str>) -> AppResult<ProfitSummary> {
    let (sales, ready_made_costs): (f64, f64) = match (from, to) {
        (Some(f), Some(t)) => conn.query_row(
            "SELECT COALESCE(SUM(total_amount),0), COALESCE(SUM(total_cost),0) FROM sales WHERE created_at BETWEEN ?1 AND ?2",
            [f, t],
            |row| Ok((row.get(0)?, row.get(1)?)),
        )?,
        _ => conn.query_row(
            "SELECT COALESCE(SUM(total_amount),0), COALESCE(SUM(total_cost),0) FROM sales",
            [],
            |row| Ok((row.get(0)?, row.get(1)?)),
        )?,
    };
